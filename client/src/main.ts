...
const fetchWeather = async (cityName: string) => {
    ...
    renderCurrentWeather(weatherData[0]); // First item assumed to be current weather
  
    const forecast = weatherData.slice(1); // The rest are assumed to be forecast
    console.log('Forecast data:', forecast); // Check if forecast data exists
...
const renderForecast = (forecasts: any[]): void => {  // ✅ Ensure forecasts is an array
  ...
  forecasts.forEach((day) => renderForecastCard(day)); // ✅ Loop through all forecasts properly

};

const renderForecastCard = (forecast: any) => {
    console.log('forecast: ', forecast); // Debugging log
    ...
    const displayIcon = icon ? `https://openweathermap.org/img/w/${icon}.png` : '';
    const displayTemp = tempF !== undefined ? `${tempF} °F` : "Temp: N/A";
    const displayWind = windSpeed !== undefined ? `Wind: ${windSpeed} MPH` : "Wind: N/A";
    const displayHumidity = humidity !== undefined ? `Humidity: ${humidity} %` : "Humidity: N/A";
  
    const { col, cardTitle, weatherIcon, tempEl, windEl, humidityEl } =
      createForecastCard();
  
    // Add content to elements
    cardTitle.textContent = displayDate;
    weatherIcon.setAttribute('src', displayIcon);
    weatherIcon.setAttribute('alt', iconDescription ?? "Weather icon");
    tempEl.textContent = displayTemp;
    windEl.textContent = displayWind;
    humidityEl.textContent = displayHumidity;
